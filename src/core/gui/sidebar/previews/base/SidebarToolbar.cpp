...
static void cb(GtkButton*, gpointer d) {
    static_cast<SidebarToolbar*>(d)->runAction(a);
}
...
SidebarToolbar::SidebarToolbar(SidebarToolbarActionListener* listener, GladeGui* gui): listener(listener) {
    ...
    g_signal_connect(this->btUp, "clicked", G_CALLBACK(cb<SIDEBAR_ACTION_MOVE_UP>), this);
    g_signal_connect(this->btDown, "clicked", G_CALLBACK(cb<SIDEBAR_ACTION_MOVE_DOWN>), this);
    g_signal_connect(this->btCopy, "clicked", G_CALLBACK(cb<SIDEBAR_ACTION_COPY>), this);
    g_signal_connect(this->btDelete, "clicked", G_CALLBACK(cb<SIDEBAR_ACTION_DELETE>), this);
}
...
void SidebarToolbarActionListener::actionPerformed(SidebarActions) {}
