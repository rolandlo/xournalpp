...
GVariant* lua_to_gvariant(lua_State* L, int idx, const GVariantType* type) {
    if (type) {
        g_message(g_variant_type_dup_string(type));
    } else {
        g_message("No type. Ignore");
    }
    switch (lua_type(L, idx)) {
        ...
        case LUA_TNIL:
            break;
        case LUA_TBOOLEAN:
            xoj_assert(type == G_VARIANT_TYPE_BOOLEAN);
        ...
        case LUA_TNUMBER:
            if (lua_isinteger(L, idx)) {
                if (type == G_VARIANT_TYPE_INT32) {
                    ...
                    g_warning("Unhandled type: %s", type ? g_variant_type_dup_string(type) : "none");
                }
            } else {
        ...
        case LUA_TTHREAD:
        default:
            g_warning("Cannot convert to GVariant");
    }
    return nullptr;
}
